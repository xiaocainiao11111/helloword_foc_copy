...
// 输入相电压，偏移量(没啥用)，电角度，转换为SVPWM马鞍波
// 简单解释：相电压等于正弦波幅值，电角度等于正弦波频率
...
float VelocityOpenLoopTick(float _target_voltageQ,float _target_angle)
{
    ...
    estimateAngle = Normalize(estimateAngle + _target_angle * deltaT); // 限幅0~2PI
    estimateVelocity = _target_angle;

    float voltageQ = _target_voltageQ;
